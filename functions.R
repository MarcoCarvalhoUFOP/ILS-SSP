...
#   #Tabela A
#   
#   data_a <- data.frame(Proposto = c(17, 16.83, 21.80, 23.07, 22.08, 25.10, 28.20, 27.95, 26.61),
#                        CS_BRKGA = c(17, 16.83, 21.80, 23.07, 22.08, 25.10, 28.20, 27.95, 26.61))
#   
...
#   #Tabela B
#   
#   data_b <- data.frame(Proposto = c(17.20, 17.37, 22.40, 24.17, 22.60, 25.40, 28.77, 29.74, 27.19),
#                        CS_BRKGA = c(17.20, 17.37, 22.40, 24.17, 22.60, 25.40, 28.77, 29.74, 27.19))
#   
...
readAllSummaryOnly <- function(filename){
  ...
  nomes = c(nomes, "GRASP", "GRASP_2OPT")
  
  allS <- read.table(paste(filename, "Summary", paste("Solution", nomes[1],".txt", sep=""), sep ="/"))
  allS <- allS[,1:5]
  names <- c("Instancia", "N_ILS_LS_PART_2OPT_1", "T_ILS_LS_PART_2OPT_1")
  for(i in 2:length(nomes))
  {
    currentSummary <- read.table(paste(filename, "Summary", paste("Solution", nomes[i],".txt", sep=""), sep ="/"))
    allS <- cbind(allS, currentSummary[,2:5]);
    names <- c(names, paste("N_",nomes[i], sep = ""), paste("T_",nomes[i], sep = ""), paste("BEST_T_",nomes[i], sep = ""), paste("ITER_",nomes[i], sep = ""))
  }
  colnames(allS) <- names
  allS
}
...
findBestResults <- function(filename, all)
{
  ...
  indice = seq(from = 2 , to = ncol(all), by = 2)
...
statisticsAntonio <- function(filepath)
{
  allSummary <- read.csv(paste(filepath, 'Resultados/AllSumary.csv', sep="/"))
  ...
  finalComp <- data.frame( matrix( nrow = nrow(uniqueDet) + 1, ncol = ncol(switchesSummary) ) ) 
  ...
  for (j in 1:ncol(switchesSummary))
  {
    finalComp[nrow(uniqueDet) + 1, j] <- sum( finalComp [ 1:nrow(uniqueDet) , j] )
  }
  
  finalComp <- cbind( rbind(uniqueDet, c(NA, NA, NA) ) , compareWith, finalComp  )
  
  write.table(finalComp[ ,1:ncol(finalComp) ], file = paste(filepath,  "Resultados", "Stats.csv", sep = "/"), dec= ",", sep=";", row.names = F)
...
{  
  ...
  colnames(proposed) <- c(colnames(compare), 'Run #1', 'Run #2', 'Run #3', 'Run #4','Run #5')
...
statisticsAntonioNumbering <- function(filepath)
{
  ...
  #{
  #  finalComp[nrow(uniqueDet) + 1, j] <- sum( finalComp [ 1:nrow(uniqueDet) , j] )
  #  finalCompTime[nrow(uniqueDet) + 1, j] <- sum( finalCompTime [ 1:nrow(uniqueDet) , j] )
  #}
